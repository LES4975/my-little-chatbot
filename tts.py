...
# -*- coding: utf-8 -*-
"""
Google Text-to-Speech API 테스트 코드
...
def main():
    ...
    try:
        ...
        while True:
            ...
            if audio_data:
                # output.mp3로 파일 저장
                saved_file = tts_client.save_audio(audio_data, "./audio_test/output.mp3")
                
                if saved_file:
                    # 자동으로 재생
                    print("\n🔊 음성 파일을 재생합니다...")
                    success = tts_client.play_with_mpg123(saved_file)
                    if not success:
                        print(f"\n💡 수동 재생 방법: mpg123 {saved_file}")
